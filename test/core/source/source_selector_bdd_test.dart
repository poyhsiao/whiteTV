...
void main() {
  ...
  group('來源選擇 BDD 場景', () {
    ...
    group('場景 1: 用戶進入詳情頁時自動選擇最快來源', () {
      test('當有多個可用來源時，選擇測速後最快的來源', () async {
        // 清除快取以確保測試獨立性
        selector.clearCache();

        final sources = [
          const VideoSource(id: 'src1', name: '量子資源', url: 'http://www.google.com', latency: 0, isAvailable: true),
          const VideoSource(id: 'src2', name: '非凡資源', url: 'http://www.cloudflare.com', latency: 0, isAvailable: true),
          const VideoSource(id: 'src3', name: '雲播資源', url: 'http://www.amazon.com', latency: 0, isAvailable: true),
        ];
        ...
        expect(selected.latency, lessThan(10000)); // 應該是可用的（< 9999）
      });
      ...
      test('當最快的來源被屏蔽時，選擇次快的來源', () async {
        ...
        // src1 被屏蔽，所以不應該選中它
        expect(selected.id, isNot('src1'));
        expect(selected.id, isIn(['src2', 'src3']));
      });
    });
    ...
    group('場景 3: 用戶屏蔽來源後不再被選擇', () {
      test('屏蔽單個來源後，該來源不會被 selectSource 返回', () async {
        ...
        expect(selected.id, isNot('src2')); // src2 被屏蔽
      });
    ...
    group('場景 4: 快取機制', () {
      test('首次選擇後結果被快取', () async {
        ...
        await selector.selectSource(sources, 'video123');
