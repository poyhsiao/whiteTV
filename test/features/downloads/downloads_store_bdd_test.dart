...
//           (目前 fake DownloadService.download 回傳 path 但沒模擬 save 到 history)
...
class _RealisticDownloadService implements DownloadService {
  final HistoryLocalService _local;
  _RealisticDownloadService(this._local);
  ...
  Future<String?> download({
    ...
    if (idx >= 0) {
      await _local.save(
        records[idx].copyWith(isDownloaded: true, localPath: path),
      );
    } else {
      await _local.save(
        PlayHistory(
...
void main() {
  ...
  group('BDD downloads.feature Scenario 1: 下載完成後出現在列表', () {
    ...
    setUp(() {
      history = _MemHistory();
      final download = _RealisticDownloadService(history);
